...
#[utoipa::path(
    post,
    path = "/api/v1/events/board/article/write-start",
    operation_id = "article/write-start",
    tag = "board",
    responses(
        (status = 201, description = "success", body = SimpleResponse)
    )
)]
#[post("article/write-start")]
async fn action(
...
#[event_type("ArticleWriteStart")]
#[schema(as = ArticleWriteStartRequest)]
pub struct Request {
    #[serde(default)]
    #[validate(required)]

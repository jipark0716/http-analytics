...
#[utoipa::path(
    post,
    path = "/api/v1/events/payment/payment-start",
    operation_id = "payment-start",
    tag = "payment",
    responses(
...
async fn action(
    ...
    let events: Vec<Event> = body.products
        .unwrap()
        .into_iter()
        .map(|product| product.new(EventType::PaymentStartItem, event_id, body.client_id, body.uuid))
        .collect();
    ...
    service
        .create_event(Event {
            ...
            event_type: EventType::PaymentStart,
            ..Default::default()
        })
        .await
        .map_err(BasicErrorErrorResponse::from)?;
